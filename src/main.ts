...
export class MermaidNode extends Node {
  ...
  public constructor(providedOptions?: MermaidNodeOptions) {
    ...
    const shapeNode =
      options.shape === "square"
        ? Rectangle.bounds(textNode.bounds.dilated(20), {
            fill: "#ccc",
            cornerRadius: 10,
          })
        : new Path(
            new Shape()
              .moveTo(textNode.centerX - diamondX, textNode.centerY)
              .lineTo(textNode.centerX, textNode.centerY - diamondY)
              .lineTo(textNode.centerX + diamondX, textNode.centerY)
              .lineTo(textNode.centerX, textNode.centerY + diamondY)
              .close(),
            {
              fill: "#ccc",
    ...
    const focusableNode = new Node({
      tagName: "div",
      focusable: true,
      accessibleName: options.text,
      accessibleHelpText: options.helpText,
    });
    ...
    focusableNode.focusHighlight = shapeNode.shape!;

    this.updateEdgesSummaryNode();
  }

  public focusContentNode(): void {
  ...
  public registerEnteringEdge(edge: MermaidDirectionalEdgeNode): void {
    if (!this.enteringEdgesNode.children.length) {
      this.enteringEdgesNode.descriptionContent = "Links entering " + this.text;
  ...
  public registerExitingEdge(edge: MermaidDirectionalEdgeNode): void {
    if (!this.exitingEdgesNode.children.length) {
      this.exitingEdgesNode.descriptionContent = `Links exiting node "${this.text}"`;
...
class EdgeNode extends Node {
  ...
  public constructor(
    ...
    this.addInputListener({
      click: () => {
        (isEntering ? edge.startNode : edge.endNode).focusContentNode();
      },
    });
  }
  ...
  public updateHighlight(): void {
    ...
    const globalShape = edgeShape.transformed(
      this.edge.getLocalToGlobalMatrix(),
    );
...
export type MermaidDirectionalEdgeNodeOptions =
  MermaidDirectionalEdgeNodeSelfOptions & NodeOptions;
...
export class MermaidDirectionalEdgeNode extends Node {
  ...
  public constructor(
    ...
    const options = optionize3<
      MermaidDirectionalEdgeNodeOptions,
      MermaidDirectionalEdgeNodeSelfOptions
    ...
    if (options.text !== null) {
      this.textNode = new Panel(
        new RichText(options.text, {
          font: new PhetFont(16),
          lineWrap: 100,
          align: "center",
          fill: "black",
  ...
  public getArrowShape(): Shape {
    ...
    return this.textNode
      ? this.textNode._background
          .shape!.transformed(this.textNode.matrix)
          .shapeUnion(this.arrow.shape!)
      : this.arrow.shape!;
  }
  ...
  public updateArrow() {
    ...
    const startPoint = MermaidDirectionalEdgeNode.getAttachmentPoint(
      this.startNode,
      this.startConnection,
    ).plus(this.startOffset);
    const endPoint = MermaidDirectionalEdgeNode.getAttachmentPoint(
      this.endNode,
      this.endConnection,
    ).plus(this.endOffset);
    ...
    const bezierEndPoint = endPoint.plus(
      endTangent.timesScalar(arrowHeadLength),
    ); // allow some overlap(?)

    const bezierControlPoint1 = bezierStartPoint.plus(
      startTangent.timesScalar(this.startTangentDistance),
    );
    const bezierControlPoint2 = bezierEndPoint.plus(
      endTangent.timesScalar(this.endTangentDistance),
    );

    const arrowHeadLeft = bezierEndPoint.plus(
      endTangent.perpendicular.timesScalar(arrowHeadWidth / 2),
    );
    const arrowHeadRight = bezierEndPoint.plus(
      endTangent.perpendicular.timesScalar(-arrowHeadWidth / 2),
    );

    const mainCubic = new Cubic(
      bezierStartPoint,
      bezierControlPoint1,
      bezierControlPoint2,
      bezierEndPoint,
    ...
    const arrowShape = new Shape([
      new Subpath([
        ...
        ...strokedRight,
        new KiteLine(
          strokedRight[strokedLeft.length - 1].end,
          strokedLeft[0].start,
        ),
      ]),
    ]);
...
const e1 = new MermaidDirectionalEdgeNode(
  doesItWorkNode,
  dontMessWithItNode,
  "left",
  "top",
...
const e2 = new MermaidDirectionalEdgeNode(
  doesItWorkNode,
  didYouMessWithItNode,
  "right",
  "top",
...
const e3 = new MermaidDirectionalEdgeNode(
  didYouMessWithItNode,
  youIdiotNode,
  "left",
  "right",
...
const e4 = new MermaidDirectionalEdgeNode(
  didYouMessWithItNode,
  willYouBeBlamedNode,
  "bottom",
  "top",
...
const e5 = new MermaidDirectionalEdgeNode(
  youIdiotNode,
  doesAnyoneElseKnowNode,
  "left",
  "right",
...
const e6 = new MermaidDirectionalEdgeNode(
  doesAnyoneElseKnowNode,
  hideItNode,
  "left",
  "top",
  { text: "No", startTangentDistance: 22 },
);
const e7 = new MermaidDirectionalEdgeNode(
  doesAnyoneElseKnowNode,
  youreToastNode,
  "bottom",
  "top",
...
const e8 = new MermaidDirectionalEdgeNode(
  willYouBeBlamedNode,
  youreToastNode,
  "left",
  "right",
...
const e9 = new MermaidDirectionalEdgeNode(
  willYouBeBlamedNode,
  forgetAboutItNode,
  "bottom",
  "top",
...
const e10 = new MermaidDirectionalEdgeNode(
  youreToastNode,
  canYouBlameNode,
  "bottom",
  "top",
);
const e11 = new MermaidDirectionalEdgeNode(
  canYouBlameNode,
  youreToastNode,
  "left",
  "left",
...
const e12 = new MermaidDirectionalEdgeNode(
  canYouBlameNode,
  noProblemNode,
  "bottom",
  "top",
...
const e13 = new MermaidDirectionalEdgeNode(
  hideItNode,
  noProblemNode,
  "bottom",
  "top",
  { endOffset: new Vector2(-30, 0) },
);
const e14 = new MermaidDirectionalEdgeNode(
  forgetAboutItNode,
  noProblemNode,
  "bottom",
  "right",
);
const e15 = new MermaidDirectionalEdgeNode(
  dontMessWithItNode,
  noProblemNode,
  "bottom",
  "left",
  { endTangentDistance: 200 },
);
...
const graphNode = new Node({
  tagName: 'div',
  accessibleHeading: 'Mermaid Flow Chart',
  descriptionContent: 'This is a demo of an accessible flow chart. It is interactive. Tab to nodes to navigate links. You can also read through the graph with typical cursor keys.',
...
layoutBoundsProperty.link((bounds) => {
  ...
  for (const node of nodes) {
    for (const edgeNode of [
      ...node.enteringEdgeNodes,
      ...node.exitingEdgeNodes,
    ]) {
